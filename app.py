...
# ── CONFIG ─────────────────────────────────────────────────────────────────────
st.set_page_config(page_title="ABMed · DIRP", page_icon="🏥", layout="wide",
...
[data-testid="stSidebar"] { background: #0D2B4E; }
[data-testid="stSidebar"] * { color: #E8EDF5 !important; }
[data-testid="stSidebar"] .stRadio label { color: #C8D4E5 !important; font-size: 0.88rem; }
[data-testid="stSidebar"] [data-baseweb="radio"] div[role="radio"][aria-checked="true"] + div { color: #fff !important; font-weight: 600; }
[data-testid="stSidebarContent"] hr { border-color: rgba(255,255,255,0.12); }
...
.abmed-header {
    ...
    box-shadow: 0 4px 20px rgba(13,43,78,0.2);
}
.abmed-header h1 { margin:0; font-size:1.4rem; font-weight:600; color:#fff; letter-spacing:-0.01em; }
.abmed-header p  { margin:0; font-size:0.82rem; color:rgba(255,255,255,0.65); margin-top:3px; }
.initiale-circle {
...
/* KPI */
.kpi-row { display:flex; gap:12px; margin-bottom:1rem; flex-wrap:wrap; }
.kpi { background:#fff; border-radius:12px; padding:1rem 1.2rem; flex:1; min-width:110px;
       box-shadow:0 1px 6px rgba(0,0,0,0.06); border-top:3px solid #1A4A7A; }
.kpi.green  { border-top-color:#1E8A52; }
.kpi.amber  { border-top-color:#D4850A; }
.kpi.red    { border-top-color:#C0392B; }
.kpi.teal   { border-top-color:#0891B2; }
.kpi-val { font-size:1.8rem; font-weight:700; color:#0D2B4E; line-height:1; }
.kpi-lbl { font-size:0.72rem; color:#8A9AB0; text-transform:uppercase; letter-spacing:0.06em; margin-top:4px; }

/* Cards missions */
.mission-card {
    background:#fff; border-radius:12px; padding:1rem 1.2rem;
    margin-bottom:0.7rem; box-shadow:0 1px 8px rgba(0,0,0,0.06);
    border-left:4px solid #1A4A7A;
}
.mission-card.controle { border-left-color:#D4850A; background:#FFFBF3; }
.mission-card.bpf      { border-left-color:#1E8A52; background:#F3FBF6; }
.card-header { display:flex; justify-content:space-between; align-items:center; margin-bottom:0.5rem; }
.card-title  { font-weight:600; font-size:0.9rem; color:#0D2B4E; }
...
.tag-ctrl    { background:#FEF3C7; color:#92400E; }
.tag-bpf     { background:#D1FAE5; color:#065F46; }
.tag-reserve { background:#FCE7F3; color:#9D174D; }
.card-meta   { display:flex; gap:1.5rem; font-size:0.8rem; color:#64748B; flex-wrap:wrap; margin-bottom:0.4rem; }
.card-structs { font-size:0.78rem; color:#475569; background:#F8FAFC;
                border-radius:8px; padding:0.4rem 0.7rem; line-height:1.5; }

/* Section titles */
.stitle { font-size:0.82rem; font-weight:600; color:#64748B; text-transform:uppercase;
          letter-spacing:0.07em; margin:1.2rem 0 0.6rem; padding-bottom:6px;
          border-bottom:1px solid #E2E8F0; }

/* Equipe chips */
.equipe-chip { display:inline-block; background:#0D2B4E; color:#fff; border-radius:6px;
               padding:3px 10px; font-size:0.78rem; font-weight:600; margin:2px; }
.equipe-chip.ctrl { background:#D4850A; }
.equipe-chip.vous { background:#1E8A52; }

/* Login */
.login-wrap { max-width:380px; margin:5rem auto; background:#fff; border-radius:16px;
              padding:2.5rem; box-shadow:0 8px 40px rgba(13,43,78,0.15); }
...
def load():
    ...
    return insp, miss, cong, ctrl

inspecteurs_df, missions_df, conges_df, controles_df = load()
VALIDES = set(inspecteurs_df["Initiale"].dropna().str.strip().str.upper())

# ── SESSION ────────────────────────────────────────────────────────────────────
for k,v in [("logged_in",False),("initiale","")]:
    if k not in st.session_state: st.session_state[k] = v

# ── SVG ICONS ──────────────────────────────────────────────────────────────────
def svg_logo():
    return """<svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
  <rect width="48" height="48" rx="12" fill="#1A4A7A"/>
  <path d="M24 10 L24 38 M10 24 L38 24" stroke="#fff" stroke-width="3.5" stroke-linecap="round"/>
  <circle cx="24" cy="24" r="6" fill="#D4850A"/>
  <circle cx="24" cy="24" r="3" fill="#fff"/>
</svg>"""

def svg_icon(name):
    icons = {
        "stats": '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><rect x="3" y="12" width="4" height="9"/><rect x="10" y="6" width="4" height="15"/><rect x="17" y="3" width="4" height="18"/></svg>',
        ...
        "globe": '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>',
        ...
        "logout": '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>',
...
def get_charge(init):
    r = inspecteurs_df[inspecteurs_df["Initiale"].str.strip().str.upper() == init]
...
def get_controles(init):
    """Missions de contrôle-poste où l'inspecteur est dans l'équipe."""
    res = []
...
def tag_html(role):
    cls = {"Principal":"tag-insp","Co-inspecteur":"tag-insp","Contrôle-poste":"tag-ctrl",
           "Réserve":"tag-reserve","BPF":"tag-bpf"}.get(role,"tag-insp")
    return f'<span class="tag {cls}">{role}</span>'

def equipe_chips(members, init_courant):
    html = ""
    for m in members:
        cls = "vous" if m == init_courant else "ctrl"
        html += f'<span class="equipe-chip {cls}">{m}</span>'
    return html
    ...
    with col_c:
        ...
        <div class="login-wrap">
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:1.5rem;">
                {svg_logo()}
                <div>
                    <div style="font-weight:700;font-size:1.1rem;color:#0D2B4E;">ABMed · DIRP</div>
                    <div style="font-size:0.78rem;color:#8A9AB0;">Programmation des inspections 2026</div>
                </div>
            </div>
            <div style="font-size:0.85rem;color:#475569;margin-bottom:1rem;">
                Renseignez vos <strong>initiales</strong> pour accéder à votre espace.
            </div>
        </div>
        """, unsafe_allow_html=True)
        init = st.text_input("Initiales", placeholder="Ex : JA, IG, SH…", max_chars=4,
                             label_visibility="collapsed").strip().upper()
        if st.button("Accéder →", use_container_width=True, type="primary"):
            if init in VALIDES:
                st.session_state.logged_in = True
                st.session_state.initiale = init
                st.rerun()
            else:
                st.error("Initiales non reconnues.")
        st.markdown(f'<p style="font-size:0.75rem;color:#aaa;text-align:center;margin-top:1rem;">{len(VALIDES)} inspecteurs · Données 2026</p>', unsafe_allow_html=True)
...
def page_dashboard():
    ...
    <div class="abmed-header">
        {svg_logo()}
        <div class="initiale-circle">{init}</div>
        <div>
            <h1>Inspecteur {init} — {groupe}</h1>
            <p>Agence Béninoise du Médicament · Direction de l'Inspection et de la Réglementation Pharmaceutique · 2026</p>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # Sidebar nav
    with st.sidebar:
        st.markdown(f"""<div style="display:flex;align-items:center;gap:10px;padding:0.5rem 0 1rem;">
            {svg_logo()}
            <div><div style="font-weight:700;font-size:0.95rem;">ABMed · DIRP</div>
            <div style="font-size:0.75rem;opacity:0.6;">{init} · {groupe}</div></div>
        </div>""", unsafe_allow_html=True)
        st.divider()

        pages = {
            "📊  Mes statistiques": "stats",
            "📅  Mes inspections": "missions",
            "🛡️  Mes contrôles-poste": "controles",
            "🏖️  Mes congés": "conges",
            "🌍  Vue globale": "globale",
            "👥  Équipe": "equipe",
        }
        choix = st.radio("", list(pages.keys()), label_visibility="collapsed")
        page = pages[choix]

        st.divider()
        if st.button("Déconnexion", use_container_width=True):
            st.session_state.logged_in = False; st.session_state.initiale = ""; st.rerun()

    # Router
    if page == "stats":      _stats(init, charge, missions, conges, controles)
    elif page == "missions": _missions(init, missions)
    elif page == "controles":_controles(init, controles)
    elif page == "conges":   _conges(conges)
    elif page == "globale":  _globale()
    elif page == "equipe":   _equipe(init)

# ── STATS ──────────────────────────────────────────────────────────────────────
def _stats(init, charge, missions, conges, controles):
    st.markdown('<div class="stitle">Indicateurs de charge</div>', unsafe_allow_html=True)
    if charge is not None:
        c1,c2,c3,c4,c5 = st.columns(5)
        vals = [
            (c1, safe_int(charge["Total"]), "Total missions", "blue"),
            (c2, safe_int(charge["P"]),     "Principal (P)",  "green"),
            (c3, safe_int(charge["Co"]),    "Co-inspecteur",  ""),
            (c4, safe_int(charge["R"]),     "Contrôles-poste","amber"),
            (c5, f'{float(charge["ICR"]):.1f}' if str(charge["ICR"]) not in ["","nan"] else "—",
                 "ICR", "teal"),
        ]
        for col, val, lbl, cls in vals:
            with col:
                st.markdown(f'<div class="kpi {cls}"><div class="kpi-val">{val}</div><div class="kpi-lbl">{lbl}</div></div>', unsafe_allow_html=True)

    if not missions.empty:
        st.markdown('<div class="stitle">Répartition par rôle</div>', unsafe_allow_html=True)
        col_a, col_b = st.columns(2)
        with col_a:
            rc = missions["Mon_Role"].value_counts().reset_index()
            rc.columns = ["Rôle","N"]
            fig = px.pie(rc, names="Rôle", values="N", hole=0.45,
                         color_discrete_sequence=["#1A4A7A","#0891B2","#D4850A","#1E8A52"])
            fig.update_layout(margin=dict(t=5,b=5,l=5,r=5), height=240,
                              legend=dict(font=dict(size=11)))
            st.plotly_chart(fig, use_container_width=True)
        with col_b:
            if "Mois" in missions.columns:
                mc = missions.groupby(["Mois","Mon_Role"]).size().reset_index(name="N")
                mc["O"] = mc["Mois"].str.upper().apply(lambda x: ORDRE_MOIS.index(x) if x in ORDRE_MOIS else 99)
                mc = mc.sort_values("O")
                fig2 = px.bar(mc, x="Mois", y="N", color="Mon_Role", barmode="stack",
                              color_discrete_map={"Principal":"#1A4A7A","Co-inspecteur":"#0891B2","Contrôle-poste":"#D4850A"},
                              labels={"N":"Missions","Mon_Role":"Rôle"})
                fig2.update_layout(margin=dict(t=5,b=5), height=240,
                                   legend=dict(font=dict(size=11)), plot_bgcolor="#fff")
                st.plotly_chart(fig2, use_container_width=True)

    if not conges.empty:
        st.markdown('<div class="stitle">Congés programmés</div>', unsafe_allow_html=True)
        for _, row in conges.iterrows():
            ...
            st.markdown(f'''<div class="conge-item">
                <strong>{mois}</strong> &nbsp;·&nbsp; {debut} → {fin}
                <span style="color:#D4850A;font-size:0.78rem;margin-left:8px;">({duree})</span><br>
                <span style="color:#64748B;">{obs if obs not in ["","nan"] else ""}</span>
            </div>''', unsafe_allow_html=True)

# ── INSPECTIONS ────────────────────────────────────────────────────────────────
def _missions(init, missions):
    st.markdown('<div class="stitle">Mes missions d\'inspection</div>', unsafe_allow_html=True)
    df = missions[missions["Mon_Role"] != "Contrôle-poste"].copy() if not missions.empty else missions

    if df.empty:
        st.info("Aucune mission d'inspection trouvée.")
        return

    col1,col2,col3 = st.columns(3)
    with col1:
        mois_list = ["Tous"] + [m for m in ORDRE_MOIS if m in df["Mois"].str.upper().values]
        fm = st.selectbox("Mois", mois_list)
    with col2:
    ...
    if fz != "Toutes" and "Zone" in df.columns: df = df[df["Zone"] == fz]

    st.caption(f"{len(df)} mission(s)")

    for _, row in df.iterrows():
        role = row.get("Mon_Role","")
        stype = str(row.get("Sous_Type","")).strip()
        card_cls = "bpf" if "BPF" in str(row.get("Type_Inspection","")).upper() else ""
        mois = str(row.get("Mois","")).strip()
        ...
        vol_disp = vol if vol not in ["","nan","None","0.0","0"] else "—"
        dept_disp = dept if dept not in ["","nan","None"] else "—"
        structs_disp = structs if structs not in ["","nan","None"] else "—"
        obs_disp = f'<div style="font-size:0.77rem;color:#94A3B8;font-style:italic;margin-top:4px;">💬 {obs}</div>' if obs not in ["","nan","None"] else ""

        st.markdown(f"""
        <div class="mission-card {card_cls}">
            <div class="card-header">
                <span class="card-title">{mois} · {sem} &nbsp;({per})</span>
                {tag_html(role)}
            </div>
            <div class="card-meta">
                <span>📍 {zone}</span>
                <span>🏷 {stype}</span>
                <span>🗺 {dept_disp}</span>
                <span>🔢 {vol_disp} structure(s)</span>
            </div>
            <div class="card-structs">{structs_disp}</div>
            {obs_disp}
        </div>
        """, unsafe_allow_html=True)

# ── CONTRÔLES-POSTE ────────────────────────────────────────────────────────────
def _controles(init, controles):
    st.markdown('<div class="stitle">Mes missions de contrôle-poste d\'enlèvement</div>', unsafe_allow_html=True)
    st.info("ℹ️ Les contrôles-poste sont des missions d'équipe. Chaque semaine, les inspecteurs en réserve forment l'équipe chargée du contrôle.", icon=None)

    if controles.empty:
        st.success("Aucun contrôle-poste programmé.")
        return
    ...
    for _, row in controles.iterrows():
        ...
        chips = equipe_chips(equipe, init)

        st.markdown(f"""
        <div class="mission-card controle">
            <div class="card-header">
                <span class="card-title">{mois} · {sem} &nbsp;({per})</span>
                <span class="tag tag-ctrl">Contrôle-poste</span>
            </div>
            <div style="margin-top:6px;">
                <span style="font-size:0.78rem;color:#92400E;font-weight:500;">Équipe :</span>&nbsp;
                {chips}
            </div>
        </div>
        """, unsafe_allow_html=True)
...
# ── VUE GLOBALE ────────────────────────────────────────────────────────────────
def _globale():
    st.markdown('<div class="stitle">Vue globale 2026</div>', unsafe_allow_html=True)
    df = missions_df.copy()
    df["Volume"] = pd.to_numeric(df["Volume"], errors="coerce").fillna(0)

    col1,col2,col3 = st.columns(3)
    with col1:
        st.markdown('<div class="kpi"><div class="kpi-val">'+str(len(df))+'</div><div class="kpi-lbl">Missions totales</div></div>', unsafe_allow_html=True)
    with col2:
        st.markdown('<div class="kpi green"><div class="kpi-val">'+str(int(df["Volume"].sum()))+'</div><div class="kpi-lbl">Structures</div></div>', unsafe_allow_html=True)
    with col3:
        st.markdown('<div class="kpi amber"><div class="kpi-val">'+str(len(controles_df))+'</div><div class="kpi-lbl">Contrôles-poste</div></div>', unsafe_allow_html=True)

    st.markdown('<div class="stitle">Volume mensuel</div>', unsafe_allow_html=True)
    by_mois = df.groupby("Mois")["Volume"].sum().reset_index()
    by_mois["O"] = by_mois["Mois"].str.upper().apply(lambda x: ORDRE_MOIS.index(x) if x in ORDRE_MOIS else 99)
    by_mois = by_mois.sort_values("O")
    fig = px.bar(by_mois, x="Mois", y="Volume", color_discrete_sequence=["#1A4A7A"])
    fig.update_layout(height=280, margin=dict(t=5,b=5), plot_bgcolor="#fff",
                      xaxis=dict(gridcolor="#F1F5F9"), yaxis=dict(gridcolor="#F1F5F9"))
    st.plotly_chart(fig, use_container_width=True)

    col_a, col_b = st.columns(2)
    with col_a:
        st.markdown('<div class="stitle">Par type</div>', unsafe_allow_html=True)
        if "Sous_Type" in df.columns:
            tc = df["Sous_Type"].value_counts().reset_index()
            tc.columns = ["Type","Missions"]
            st.dataframe(tc, use_container_width=True, height=180)
    with col_b:
        st.markdown('<div class="stitle">Par zone</div>', unsafe_allow_html=True)
        if "Zone" in df.columns:
            zc = df["Zone"].value_counts().reset_index()
            zc.columns = ["Zone","Missions"]
...
def _equipe(init_courant):
    st.markdown('<div class="stitle">Charge de l\'équipe</div>', unsafe_allow_html=True)
    df = inspecteurs_df.copy()
    df["ICR_n"] = pd.to_numeric(df["ICR"], errors="coerce")
    df[""] = df["Initiale"].apply(lambda x: "⬅ vous" if str(x).strip().upper() == init_courant else "")
    cols = [c for c in ["Initiale","Groupe","P","Co","R","Total","IC","ICR",""] if c in df.columns]
    ...
    colors = ["#1E8A52" if str(x).strip().upper() == init_courant else "#1A4A7A" for x in s["Initiale"]]
    fig = go.Figure(go.Bar(x=s["ICR_n"], y=s["Initiale"], orientation="h",
                           marker_color=colors, text=s["ICR_n"], textposition="outside"))
    fig.update_layout(height=max(320,len(df)*22), margin=dict(l=5,r=40,t=5,b=5),
                      plot_bgcolor="#fff", xaxis=dict(gridcolor="#F1F5F9"))
    st.plotly_chart(fig, use_container_width=True)

    st.markdown('<div class="stitle">Congés équipe</div>', unsafe_allow_html=True)
    cols_cg = [c for c in ["Mois","Initiale","Nom_Complet","Date_Debut","Date_Fin","Duree","Zone_Impact"] if c in conges_df.columns]
    st.dataframe(conges_df[cols_cg].reset_index(drop=True), use_container_width=True, height=280)
